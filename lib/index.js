...
module.exports = function modelBase (bookshelf, params) {
  ...
  var model = bookshelf.Model.extend({
    ...
    validateSave: function (model, attrs, options) {
      var validation
      var children = this.validate.describe().children
      var strippedKeys = []
      var dependentKeys = []
      Object.keys(children).forEach(function (key) {
        var flags = children[key].flags
        var alternatives = children[key].alternatives
        if(flags && flags.strip) strippedKeys.push(key)
        if(alternatives) {
          alternatives.forEach(function (alternative) {
            // TODO: Are there other kinds of refs?
            var refKey = alternative.ref.match(/ref:(.*)/)[1]
            dependentKeys.push(refKey)
          })
        }
      })
      ...
      if ((model && !model.isNew()) || (options && (options.method === 'update' || options.patch === true))) {
        ...
        var stripped = pick(model.attributes, strippedKeys)
        var dependents = pick(model.attributes, dependentKeys)
        // TODO: Test precendence of these values. Unclear what happens if, e.g.
        // a stripped, dependent value has been changed.
        var affectedAttributes = Object.assign({}, stripped, dependents, model.changed)
        // only validate the keys that are being updated
        validation = Joi.validate(affectedAttributes, this.validate.optionalKeys(optionalKeys), joiOpts)
      } else {
        validation = Joi.validate(this.attributes, this.validate, joiOpts)
      }
      ...
      if (validation.error) {
        ...
        strippedKeys.forEach(function (stripped) { delete model.attributes[stripped] })
