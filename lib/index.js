...
module.exports = function modelBase (bookshelf, params) {
  ...
  var model = bookshelf.Model.extend({
    ...
    validateSave: function (model, attrs, options) {
      ...
      var strippedAttrs = Object.keys(children).filter(function (key) {
        var flags = children[key].flags
        return flags && flags.strip
      })
      ...
      if ((model && !model.isNew()) || (options && (options.method === 'update' || options.patch === true))) {
        ...
        var strippedValues = pick(model.attributes, strippedAttrs)
        var changedOrEphemeral = Object.assign({}, strippedValues, model.changed)
        // only validate the keys that are being updated
        validation = Joi.validate(changedOrEphemeral, this.validate.optionalKeys(optionalKeys), joiOpts)
      } else {
      ...
      if (validation.error) {
        ...
        strippedAttrs.forEach(function (stripped) { delete model.attributes[stripped] })
